...
def load_model(
    ...
    model = Model(
        backbone_name,
        p=0.5, # p=p
        n_mels=128, # n_mels=n_mels
        num_class=397, # num_class=num_class 
...
def prediction_for_clip(test_df: pd.DataFrame, clip: np.ndarray, models, gpu_id="0"):
    ...
    for image, row_id, target in tqdm(loader):
        ...
        with torch.no_grad():
            ...
            proba = proba.mean(0) # TODO 这块非常重要，四个model的预测结果的mean!
        probas.append(proba)
    ...
    for i, (trn_idx, val_idx) in enumerate(
        ...
        for j in trange(len(valid_df_fold)):
            ...
            npy_filepath = os.path.join(
                f"../../input/birdclef-2021/debug_pseudo_label_stage1_{args.weight_stage1}",
                primary_label,
                filename.split(".")[0],
            )
            os.makedirs(
